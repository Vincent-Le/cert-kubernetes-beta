...
function show_tips_es_to_os_migration(){
    ...
    echo "        -include=<comma separated indices>   List of indices to include will be migrated"
    echo "        -exclude=<comma separated indices>   List of indices to exclude will not be migrated"
    echo "        -include_regex=<regex pattern>       Regex pattern to include indices will be migrated"
    echo "        -exclude_regex=<regex pattern>       Regex pattern to exclude indices will not be migrated"
    ...
    echo "        execute commands:"
    echo "        ${GREEN_TEXT}# $OPENSEARCH_MIGRATION_SCRIPT -starttime=\"2023-01-28T12:00:00\" -endtime=\"2024-02-07T00:00:00\"${RESET_TEXT}"
...
function show_help() {
    ...
    echo "  -m  The valid mode types are:[upgradeOperator], [upgradeOperatorStatus], [upgradeDeployment], [upgradeDeploymentStatus] and [upgradePostconfig]"
    ...
    echo "      - STEP 1 (Required): Run the script in [upgradeOperator] mode to upgrade CP4BA operators/migrate (Cluster-scoped -> Cluster-scoped / Cluster-scoped -> Namespace-scoped / Namespace-scoped -> Namespace-scoped) the IBM Cloud Pak foundational services and then shutdown all CP4BA operators before upgrade CP4BA deployment."
...
function parse_arguments() {
    ...
    while [[ "$@" != "" ]]; do
        ...
        -m)
            ...
            RUNTIME_MODE=$1
            if [[ $RUNTIME_MODE == "upgradeOperator" || $RUNTIME_MODE == "upgradeOperatorStatus" || $RUNTIME_MODE == "upgradeDeployment" || $RUNTIME_MODE == "upgradeDeploymentStatus" || $RUNTIME_MODE == "upgradePostconfig" ]]; then
                echo -n
            else
                msg "Use a valid value: -m [upgradeOperator] or [upgradeOperatorStatus] or [upgradeDeployment] or [upgradeDeploymentStatus] or [upgradePostconfig]"
                exit -1
            fi
            ;;
        # -s)
        ...
        if [[ $READY_FOR_DIRECT_UPGRADE == "Yes" ]]; then
            ...
            if [[ "$cp4a_operator_csv_version" == "21.3."* || "$cp4a_operator_csv_version" == "22.2."* ]]; then
                ...
                if [[ $UPGRADE_MODE == "dedicated2dedicated" && $ENABLE_PRIVATE_CATALOG -eq 1 ]]; then
                    ...
                    # keep GCN catalog
                    msg "All arguments passed into the CPfs script: $COMMON_SERVICES_SCRIPT_FOLDER/migrate_tenant.sh --operator-namespace ibm-common-services --services-namespace $TARGET_PROJECT_NAME --cert-manager-source ibm-cert-manager-catalog --licensing-source ibm-licensing-catalog --enable-licensing --yq \"$CPFS_YQ_PATH\" -c $CS_CHANNEL_VERSION -s $CS_CATALOG_VERSION --license-accept"
                    $COMMON_SERVICES_SCRIPT_FOLDER/migrate_tenant.sh --operator-namespace ibm-common-services --cert-manager-source ibm-cert-manager-catalog --licensing-source ibm-licensing-catalog --enable-licensing --yq "$CPFS_YQ_PATH" -c $CS_CHANNEL_VERSION -s $CS_CATALOG_VERSION --license-accept
                    if [ $? -ne 0 ]; then
                        warning "Failed to execute command: $COMMON_SERVICES_SCRIPT_FOLDER/migrate_tenant.sh --operator-namespace ibm-common-services --services-namespace $TARGET_PROJECT_NAME --cert-manager-source ibm-cert-manager-catalog --licensing-source ibm-licensing-catalog --enable-licensing --yq \"$CPFS_YQ_PATH\" -c $CS_CHANNEL_VERSION -s $CS_CATALOG_VERSION --license-accept"
                        echo "${YELLOW_TEXT}[ATTENTION]:${RESET_TEXT} You can run follow command to try upgrade again after fix migration issue of IBM Cloud Pak foundational services."
                        echo "           ${GREEN_TEXT}# ./cp4a-deployment.sh -m upgradeOperator -n $TARGET_PROJECT_NAME --cpfs-upgrade-mode <migration mode> --original-cp4ba-csv-ver <cp4ba-csv-version-before-upgrade>${RESET_TEXT}"
                        echo "           Usage:"
                        echo "           --cpfs-upgrade-mode     : The migration mode for IBM Cloud Pak foundational services, the valid values [shared2shared/shared2dedicated/dedicated2dedicated]"
